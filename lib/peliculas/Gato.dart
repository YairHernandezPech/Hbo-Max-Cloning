...
class _GatoState extends State<Gato> {
  ...
  void _checkFavorite() async {
    bool favorite = await DatabaseHelper.instance.isFavorite('Gato con Botas');
    setState(() {
      isFavorite = favorite;
    });
  }
  ...
  void _toggleFavorite() async {
    if (isFavorite) {
      await DatabaseHelper.instance.removeFavorite('Gato con Botas');
    } else {
      await DatabaseHelper.instance.addFavorite(
        'Gato con Botas',
        'images/gato2.jpeg',
        'El Gato con Botas descubre su pasión por la aventura le ha costado ocho de sus nueve vidas. Así que debe partir en busca de la Estrella de los Deseos.',
      );
    }
    _checkFavorite();
  }
                          ...
                          Column(
                            children: [
                              IconButton(
                                ...
                                onPressed: () async {
                                  await Share.share(
                                      'Hey! Mira esta película en HBO Max: Gato con Botas https://www.hbomax.com/ve/es/movies/ttl0c0d0d0gatoconbotas');
                                },
                              ),
                              const Text(
                                'Comparte',
...
class _YoutubePlayerExampleState extends State<YoutubePlayerExample> {
  ...
  void initState() {
    ...
    _controller = YoutubePlayerController(
      initialVideoId: 'HmF0OtVRfYg', // ID del video de YouTube
